		...
		if (linHSolSeq->Length() > 0) //only linear half spaces are treated
		{
			...
			for (int k=1; k<=linHSolSeq->Length() ; k++) // make huge finit solids which represent the half spaces
			{
				...
				try
				{
					...
					pDF.SetShapes(tmpS1, Big);
					pDF.Perform();

					BRepAlgoAPI_Common aCommonOp(tmpS1,Big, pDF);
			...
			for (int k=1; k<=HSolSeq->Length() ; k++)
			{
				...
				while(need2repeat)
				{
					try
					{
						...
						BRepAlgoAPI_Common aCommonOp(oldSol1,polSol1, pDF);
		...
		//## process all failed solids
			...
			//## compute boxed half space
				while(bopFailed)
				{
					...
					{
						BOPTools_DSFiller pDF;
						pDF.SetShapes(tmpShp, HSolSeq->Value(k));
						pDF.Perform();

						BRepAlgoAPI_Common outerCommonOp(tmpShp,HSolSeq->Value(k), pDF);
			...
			//## compute common of polSol1 and tmpS2
				...
				{
					BOPTools_DSFiller pDF;
					pDF.SetShapes(tmpS2, polSol1);
					pDF.Perform();

					BRepAlgoAPI_Common theCOp(tmpS2, polSol1, pDF);
